...
function DataType() {
  return (
    <div style={{ padding: 20 }}>
      ...
      <h2>2.Array</h2>
      <p >0.文档地址:<a href="https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array" target="_blank">https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array</a></p>
      <p >1.push:在数组的尾部添加一个或者几个元素,返回添加元素的最后一个,原数组会改变</p>
      <p >2.pop:数组尾部删除一个元素,返回被删除的元素,原数组会改变</p>
      <p >3.shift:删除数组的子一个元素,返回被删除的元素,原数组会改变</p>
      <p >4.unshift:在数组的起始位置添加一个元素,返回数组长度,原数组会改变</p>
      <p >5.splice:从数组的指定位置起删除固定长度的元素,返回被删除的元素,原数组会改变</p>
      <p >6.slice:指定起始位置和终止位置,复制一个数组,不传参时复制整个数组,原数组不会改变(可以用来截断字符串),不包括终止位置</p>
      <p >7.join:数组转字符串,参数为分割字符串</p>
      <p >8.copyWithin:复制数组的一部分到另外一部分(覆盖),数组的总长度不变</p>
      <p >9.concat:链接数组,返回一个新的数组(数组合并去重可以使用解构赋值的方式,例如:[...new Set([...a,...b])])</p>
      <p >10.every:返回一个boolean值,表示是否数组的每一项执行测试函数时,都返回true</p>
      <p >11.some:返回一个boolean值,表示至少有一个元素为true</p>
      <p >12.find/findIndex:返回满足测试函数的第一个值/第一个值的Index.若没有则返回undefined/-1</p>
      <p >13.entries/keys/values:跟Object的entries/keys/values差不多</p>
      <p >14.filter/foreach:过滤/遍历</p>
      <p >15.sort/reverse:排序和反转</p>
      <p >16.fill:用一个固定值填充,数组起始位置到终止位置的所有值</p>
      <p >17.flat/flatMap:指定层数的扁平化,默认为1层/映射每一个元素,然后将结果压缩成一个新数组</p>
      <p >18.includes:判断数组中是否包含指定值</p>
      <p >19.map/reduce/reduceRight:映射/升序执行reduce函数,汇总返回为一个值/接受一个函数作为累加器（accumulator）和数组的每个值（从右到左）将其减少为单个值</p>
      <p >20.静态方法:isArray,from,of</p>
      ...
      <h2>4.Symbol</h2>
      <p >0.<a href="https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/Symbol">文档地址</a></p>
      <p >待补充</p>
